...
from utils.images import validate_image_file, save_dog_image
from repo.dog_repo import DogRepo


def init_dog_routes(app, db: DB, repo: DogRepo):
    ...
    def update_dog():
        ...
        try:
            ...
            name = data.get(
                "name"
            ).lower()  # New name (or current name if not renaming)
            old_name = data.get(
                "old_name", name
            ).lower()  # If old_name provided, use it; otherwise assume name is the identifier
            birth_date = date.fromisoformat(data.get("birth_date"))
        ...
        try:
            ...
            dog_dto = repo._to_dto(updated_dog)

            return jsonify({"message": "Dog successfully updated", "dog": dog_dto}), 200
